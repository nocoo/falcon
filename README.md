...
v0.1.4 是供个人使用的 macOS 15+、Apple Silicon 专用 DMG，不支持 Intel Mac，采用自己账户的 Apple Development 签名，未经过 Apple 公证。从 [GitHub Releases](https://github.com/nocoo/falcon/releases) 下载 DMG 和同名 `.sha256`，在下载目录执行：

```sh
shasum -a 256 -c Falcon-0.1.4-arm64.dmg.sha256
```
