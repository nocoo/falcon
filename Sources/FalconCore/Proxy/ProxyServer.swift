...
public actor ProxyServer {
    ...
    private func didBind(_ actualPort: Int?) {
        ...
        for waiter in bindWaiters { waiter.resume(returning: actualPort) }
    ...
    private func didStop(_ error: Error?) {
        ...
        for waiter in bindWaiters { waiter.resume(throwing: failure) }
        bindWaiters.removeAll()
        task = nil
    }
    ...
    private func respond(_ request: Hummingbird.Request) async -> Hummingbird.Response {
        ...
        guard let authorization = request.headers[.authorization], authorization.hasPrefix("Bearer "),
            !authorization.dropFirst(7).isEmpty
        else { return Self.failure(401, "unauthorized") }
        let token = String(authorization.dropFirst(7))
        do { _ = try await configuration.authenticate(token: token) } catch {
            if (error as? FalconError)?.status == 403 { return Self.failure(403, "source_disabled") }
            return Self.failure(401, "unauthorized")
        }

        return await route(request, token: token, receivedAt: receivedAt, started: started)
    }

    private func route(
        _ request: Hummingbird.Request, token: String, receivedAt: Date, started: ContinuousClock.Instant
    ) async -> Hummingbird.Response {
        ...
        let body: Data
        do { body = try await Self.readBody(request.body, until: started.advanced(by: .seconds(30))) } catch {
            let failure = error as? FalconError
            let reply = await service.rejectIncomplete(
                token: token, code: failure?.code ?? "body_incomplete", status: failure?.status ?? 400,
        ...
        let reply = await service.submit(
            token: token, body: body, metadata: metadata, receivedAt: receivedAt, receivedInstant: started)
        return Self.response(reply, service: service)
    }

    private func mcp(
        request: Hummingbird.Request, body: Data, token: String, receivedAt: Date, started: ContinuousClock.Instant
    ) async -> Hummingbird.Response {
        ...
        if reply.status >= 400 {
            ...
            return .object([
                ...
                "error": .object([
                    ...
                    "retryable": .bool(false), "outcome_unknown": .bool(error.outcomeUnknown),
                ]),
            ])
        } else {
            return .object([
    ...
    private static func failure(_ status: Int, _ code: String) -> Hummingbird.Response {
        let errorFields: [String: JSONValue] = [
            "origin": .string("falcon"), "code": .string(code), "message": .string(code),
    ...
    private static func response(_ reply: ProxyReply, service: DecisionService) -> Hummingbird.Response {
        ...
        return response(
            reply.status, body: reply.body, headers: headers,
            delivery: reply.requestID.flatMap { id in reply.receivedInstant.map { (id, $0) } }, service: service)
    }

    private static func response(
        _ status: Int, body: Data, headers: [String: String] = [:], delivery: (UUID, ContinuousClock.Instant)? = nil,
        service: DecisionService? = nil
...
enum JevMCPTool {
    ...
    private static func inputSchema() -> JSONValue {
        let structured: JSONValue = .object([
            "oneOf": .array(["string", "object", "array"].map { .object(["type": .string($0)]) })
        ])
        let description: JSONValue = .object([
            "oneOf": .array(["string", "object", "array", "null"].map { .object(["type": .string($0)]) })
        ])
        let variants: [JSONValue] = [
            .object([
                "type": .string("object"),
                "required": .array([.string("type"), .string("instructions"), .string("criteria")]),
                "properties": .object([
                    "type": .object(["const": .string("choice")]), "instructions": structured,
                    "criteria": .object([
                        "type": .string("object"), "minProperties": .number(2), "maxProperties": .number(255),
                        "additionalProperties": description,
                    ]),
                ]), "additionalProperties": .bool(true),
            ]),
            .object([
                "type": .string("object"), "required": .array([.string("type"), .string("instructions")]),
                "properties": .object([
                    "type": .object(["const": .string("noul")]), "instructions": structured,
                    "criteria": .object([
                        "type": .string("object"), "required": .array([.string("true"), .string("false")]),
            ...
            .object([
                ...
                "required": .array([.string("type"), .string("instructions"), .string("criteria")]),
                "properties": .object([
                    "type": .object(["const": .string("score")]), "instructions": structured,
                    "criteria": .object([
                        "type": .string("array"), "minItems": .number(2), "maxItems": .number(10), "items": description,
                    ]),
                ]), "additionalProperties": .bool(true),
            ]),
        ]
        return .object([
            ...
            "properties": .object([
                "state": structured, "model": .object(["type": .string("string")]),
                "questions": .object([
                    "type": .string("object"), "additionalProperties": .object(["oneOf": .array(variants)]),
                ]),
            ]), "additionalProperties": .bool(true),
        ])
    }

    private static func outputSchema() -> JSONValue {
        .object([
            "type": .string("object"),
            "properties": .object([
                "request_id": .object(["type": .string("string")]), "response": .object(["type": .string("object")]),
