...
public actor DecisionStore {
    ...
    public init(path: String, testRunID: UUID? = nil, budgetBytes: Int64 = FalconLimits.storageBytes) throws {
        ...
        guard fd >= 0 else { throw FalconError("storage_unavailable", "Unable to open Falcon database lock.", status: 507) }
        ...
        defer {
            if !initialized { flock(fd, LOCK_UN); close(fd) }
        }
        lockFD = fd
        ...
        try db.write { database in
            ...
            if let testRunID {
                ...
                if let marker {
                    guard marker == testRunID.uuidString else { throw FalconError("test_marker_mismatch", "Test database marker does not match.") }
                } else {
                    guard !existed else { throw FalconError("test_marker_missing", "Existing database has no test marker.") }
                    try database.execute(sql: "INSERT INTO _test_marker(run_id) VALUES (?)", arguments: [testRunID.uuidString])
                }
            } else {
                let marker: String? = try String.fetchOne(database, sql: "SELECT run_id FROM _test_marker")
                guard marker == nil else { throw FalconError("test_database", "Test database cannot be opened as production data.") }
            }
        }
    ...
    public func saveProfile(_ profile: UpstreamProfile) throws {
        ...
        try managedWrite(131_072) { database in
            try database.execute(sql: """
                ...
                """, arguments: [profile.id.uuidString, profile.name, profile.baseURL, profile.defaultModel,
                                   profile.revision, profile.credentialID, profile.enabled])
        }
    }
    ...
    public func saveSource(_ source: AgentSource) throws {
        ...
        try managedWrite(131_072) { database in
            try database.execute(sql: """
                ...
                """, arguments: [source.id.uuidString, source.name, source.profileID.uuidString,
                                   source.enabled, source.archived, source.createdAt.timeIntervalSince1970])
        }
    }

    public func createSourceWithKey(_ source: AgentSource, key: SourceKey) throws {
        try checkName(source.name)
        guard key.sourceID == source.id, key.digest.count == 32, key.suffix.count == 4,
              key.revokedAt == nil else { throw FalconError("invalid_key", "Invalid source key.") }
        try managedWrite(131_072) { database in
            try database.execute(sql: "INSERT INTO sources VALUES (?, ?, ?, ?, ?, ?)",
                                 arguments: [source.id.uuidString, source.name, source.profileID.uuidString,
                                             source.enabled, source.archived, source.createdAt.timeIntervalSince1970])
            try database.execute(sql: "INSERT INTO source_keys VALUES (?, ?, ?, ?, ?, NULL)",
                                 arguments: [key.id.uuidString, source.id.uuidString, key.digest,
                                             key.suffix, key.createdAt.timeIntervalSince1970])
        }
    }
    ...
    public func configurationSnapshot(tokenDigest: Data) throws -> (SourceIdentity, UpstreamProfile)? {
        try db.read { database in
            ...
            guard let key = matched,
                  let sourceRow = try Row.fetchOne(database, sql: "SELECT * FROM sources WHERE id=?", arguments: [key.sourceID.uuidString]) else { return nil }
            let source = Self.source(sourceRow)
            guard let profileRow = try Row.fetchOne(database, sql: "SELECT * FROM upstream_profiles WHERE id=?", arguments: [source.profileID.uuidString]) else { return nil }
            return (SourceIdentity(source: source, keyID: key.id), Self.profile(profileRow))
        }
    }

    public func replaceKey(_ key: SourceKey) throws {
        guard key.digest.count == 32, key.suffix.count == 4 else { throw FalconError("invalid_key", "Invalid source key.") }
        try managedWrite(131_072) { database in
            try database.execute(sql: "UPDATE source_keys SET revoked_at=? WHERE source_id=? AND revoked_at IS NULL",
                                 arguments: [key.createdAt.timeIntervalSince1970, key.sourceID.uuidString])
            try database.execute(sql: "INSERT INTO source_keys VALUES (?, ?, ?, ?, ?, NULL)",
                                 arguments: [key.id.uuidString, key.sourceID.uuidString, key.digest,
                                             key.suffix, key.createdAt.timeIntervalSince1970])
        }
    }
    ...
    public func reserve(requestID: UUID, requestBytes: Int, questionCount: Int) throws {
        guard !blockedIDs.contains(requestID), reservations[requestID] == nil,
              requestBytes >= 0, requestBytes <= FalconLimits.requestBytes,
              questionCount >= 0, questionCount <= requestBytes else {
            throw FalconError("invalid_reservation", "Invalid request reservation.", status: 507)
        }
    ...
    public func insert(_ detail: RequestDetail) throws {
        ...
        guard let reservation = reservations[detail.id], reservation.generation == generation,
              detail.summary.questionCount <= reservation.questionCount,
              !blockedIDs.contains(detail.id), detail.summary.expiresAt > Date() else {
            throw FalconError("request_not_reserved", "Request cannot be inserted.", status: 507)
        }
        try db.write { database in
            try Self.write(detail, database: database, insert: true)
        }
    }
    ...
    public func update(_ detail: RequestDetail) throws {
        ...
        guard let reservation = reservations[detail.id], reservation.generation == generation,
              detail.summary.questionCount <= reservation.questionCount,
              !blockedIDs.contains(detail.id) else { throw FalconError("request_cleared", "Request was cleared or not reserved.") }
        try db.write { database in
            guard let row = try Row.fetchOne(database, sql: "SELECT summary_json FROM requests WHERE id=? AND expires_at>?",
                                             arguments: [detail.id.uuidString, Date().timeIntervalSince1970]) else {
                throw FalconError("request_missing", "Request no longer exists.")
            }
            ...
            guard old.id == next.id, old.sourceID == next.sourceID, old.keyID == next.keyID,
                  ...
                  old.receivedAt == next.receivedAt, old.expiresAt == next.expiresAt,
                  !old.status.isTerminal else {
                throw FalconError("invalid_transition", "Request identity or terminal status cannot change.")
            }
            var merged = detail
    ...
    public func updateDelivery(id: UUID, state: DeliveryState, finishedMS: Double?) throws {
        ...
        try managedWrite(131_072) { database in
            guard let row = try Row.fetchOne(database, sql: "SELECT summary_json FROM requests WHERE id=? AND expires_at>?",
                                             arguments: [id.uuidString, Date().timeIntervalSince1970]) else { return }
            var summary = try JSONDecoder().decode(RequestSummary.self, from: row["summary_json"] as Data)
            guard summary.status.isTerminal else { throw FalconError("invalid_delivery", "Delivery requires a terminal request.") }
            guard state != .written || finishedMS != nil else { throw FalconError("invalid_delivery", "Written delivery requires timing.") }
            summary.delivery = state
            summary.timing.deliveryFinishedMS = finishedMS
            try database.execute(sql: "UPDATE requests SET summary_json=? WHERE id=?",
                                 arguments: [try JSONEncoder().encode(summary), id.uuidString])
        }
    }

    public func requests(filter: DecisionFilter = .init(), before: RequestCursor? = nil, limit: Int = 100, now: Date = Date()) async throws -> [RequestSummary] {
        guard (1...100).contains(limit) else { throw FalconError("invalid_limit", "Page limit must be 1 to 100.") }
        let queue = db
        return try await withTaskCancellationHandler {
            try Task.checkCancellation()
            return try queue.read { database in
                let (whereSQL, arguments) = try Self.predicate(filter, before: before, now: max(now, Date()))
                return try Row.fetchAll(database, sql: "SELECT summary_json FROM requests WHERE \(whereSQL) ORDER BY received_at DESC, id DESC LIMIT ?",
                                        arguments: StatementArguments(arguments + [limit])).map {
                    try JSONDecoder().decode(RequestSummary.self, from: $0["summary_json"] as Data)
                }
            }
    ...
    public func detail(id: UUID, now: Date = Date()) throws -> RequestDetail? {
        try db.read { database in
            guard let row = try Row.fetchOne(database, sql: "SELECT * FROM requests WHERE id=? AND expires_at>?",
                                             arguments: [id.uuidString, max(now, Date()).timeIntervalSince1970]) else { return nil }
            return RequestDetail(summary: try JSONDecoder().decode(RequestSummary.self, from: row["summary_json"] as Data),
                                 receivedRequest: row["received_request"], effectiveRequest: row["effective_request"],
                                 upstreamResponse: row["upstream_response"])
        }
    }

    public func replayCandidates(filter: DecisionFilter, limit: Int = 10_000, now: Date = Date()) async throws -> [RequestSummary] {
        guard (1...10_000).contains(limit) else { throw FalconError("invalid_limit", "Replay limit must be 1 to 10000.") }
        let queue = db
        return try await withTaskCancellationHandler {
            try Task.checkCancellation()
            return try queue.read { database in
                let (whereSQL, arguments) = try Self.predicate(filter, before: nil, now: max(now, Date()))
                let terminal = "status NOT IN ('accepted', 'in_flight')"
                let rows = try Row.fetchAll(database, sql: "SELECT summary_json FROM requests WHERE \(whereSQL) AND \(terminal) ORDER BY received_at, id LIMIT ?",
    ...
    public func usage(filter: DecisionFilter = .init(), now: Date = Date()) async throws -> UsageSnapshot {
        ...
        return try await withTaskCancellationHandler {
            ...
            let (whereSQL, arguments) = try Self.predicate(filter, before: nil, now: current)
            let cursor = try Row.fetchCursor(database, sql: "SELECT summary_json FROM requests WHERE \(whereSQL)", arguments: StatementArguments(arguments))
            var result = UsageSnapshot()
            var latencies: [Double] = []
            var returnLatencies: [Double] = []
            var buckets: [Date: UsageBucket] = [:]
            var sources: [UUID: SourceUsage] = [:]
            var models: [String: Int] = [:]
            if let since = filter.since?.timeIntervalSince1970,
               let until = filter.until?.timeIntervalSince1970,
               since.isFinite, until.isFinite, until > since {
                ...
                if end > start {
                    ...
                    let first = Int(floor(start / step))
                    let last = Int(floor(end.nextDown / step))
            ...
            while let row = try cursor.next() {
                try Task.checkCancellation()
                let summary = try JSONDecoder().decode(RequestSummary.self, from: row["summary_json"] as Data)
                result.requests += 1
                ...
                if summary.status.isTerminal {
                    ...
                    if let ms = summary.timing.terminalMS, ms.isFinite, ms >= 0 { latencies.append(ms) }
                    else { result.missingLatency += 1 }
                    if summary.delivery == .written,
                       let ms = summary.timing.deliveryFinishedMS, ms.isFinite, ms >= 0 { returnLatencies.append(ms) }
                    else { result.missingReturnLatency += 1 }
                }
                ...
                let date = Date(timeIntervalSince1970: floor(summary.receivedAt.timeIntervalSince1970 / result.bucketSeconds) * result.bucketSeconds)
                ...
                buckets[date] = bucket
                var source = sources[summary.sourceID] ?? SourceUsage(id: summary.sourceID, name: summary.sourceName, requests: 0)
                source.requests += 1
            ...
            latencies.sort()
            returnLatencies.sort()
            result.latencySamples = latencies.count
            result.returnLatencySamples = returnLatencies.count
            if !latencies.isEmpty {
                result.p50MS = latencies[Int(ceil(0.5 * Double(latencies.count))) - 1]
                result.p95MS = latencies[Int(ceil(0.95 * Double(latencies.count))) - 1]
            }
            if !returnLatencies.isEmpty {
                result.returnP50MS = returnLatencies[Int(ceil(0.5 * Double(returnLatencies.count))) - 1]
                result.returnP95MS = returnLatencies[Int(ceil(0.95 * Double(returnLatencies.count))) - 1]
            }
            result.processingLatencies = Self.latencyBuckets(latencies)
            result.returnLatencies = Self.latencyBuckets(returnLatencies)
            result.models = models.map { CategoryUsage(name: $0.key, count: $0.value) }.sorted { $0.count == $1.count ? $0.name < $1.name : $0.count > $1.count }
            ...
            let questionCursor = try Row.fetchCursor(database, sql: "SELECT q.type, q.confidence, q.numeric_value, requests.status \(questionSQL)", arguments: StatementArguments(arguments))
            var types: [String: Int] = [:]
            var confidence = Array(repeating: 0, count: 10)
            var noul = Array(repeating: 0, count: 10)
            while let row = try questionCursor.next() {
                try Task.checkCancellation()
                ...
                if type == "choice" || type == "score" {
                    ...
                    if let value, value.isFinite, (0...1).contains(value) { confidence[min(9, Int(value * 10))] += 1 }
                    else { result.missingConfidence += 1 }
                } else if type == "noul" {
                    let value: Double? = row["numeric_value"]
                    if let value, value.isFinite, (0...1).contains(value) { noul[min(9, Int(value * 10))] += 1 }
                    else { result.missingNoul += 1 }
                }
            }
            result.questionTypes = types.map { CategoryUsage(name: $0.key, count: $0.value) }.sorted { $0.count == $1.count ? $0.name < $1.name : $0.count > $1.count }
            ...
            let succeededSQL = "\(questionSQL) AND requests.status='succeeded'"
            let groupCount = try Int.fetchOne(database, sql: "SELECT COUNT(DISTINCT q.fingerprint) \(succeededSQL)", arguments: StatementArguments(arguments)) ?? 0
            result.omittedDecisionGroups = max(0, groupCount - 100)
            let groupRows = try Row.fetchAll(database, sql: """
            ...
            for row in groupRows {
                ...
                if type == "choice" {
                    let outcomeRows = try Row.fetchAll(database, sql: """
                ...
                result.decisionGroups.append(DecisionGroup(fingerprint: row["fingerprint_hex"],
                    questionID: row["question_id"], type: type, samples: row["samples"], outcomes: outcomes,
                    mean: row["mean"], minimum: row["minimum"], maximum: row["maximum"]))
            }
            return result
            }
        } onCancel: { queue.interrupt() }
    ...
    public func saveReview(id: UUID, state: ReviewState, note: String, now: Date = Date()) throws {
        ...
        try managedWrite(262_144) { database in
            guard let row = try Row.fetchOne(database, sql: "SELECT summary_json FROM requests WHERE id=? AND expires_at>?",
                                             arguments: [id.uuidString, max(now, Date()).timeIntervalSince1970]) else {
                throw FalconError("request_missing", "Request no longer exists.")
            }
            var summary = try JSONDecoder().decode(RequestSummary.self, from: row["summary_json"] as Data)
            summary.reviewState = state
            summary.reviewNote = note
            try database.execute(sql: "INSERT INTO reviews VALUES (?, ?, ?, ?) ON CONFLICT(request_id) DO UPDATE SET state=excluded.state, note=excluded.note, updated_at=excluded.updated_at",
                                 arguments: [id.uuidString, state.rawValue, note, max(now, Date()).timeIntervalSince1970])
            try database.execute(sql: "UPDATE requests SET review_state=?, summary_json=? WHERE id=?",
    ...
    public func cleanup(now: Date = Date()) throws {
        try db.write { database in
            try database.execute(sql: "DELETE FROM requests WHERE expires_at<=?", arguments: [max(now, Date()).timeIntervalSince1970])
        }
        try reclaimPages()
    }
    ...
    public func recoverInterrupted(now: Date = Date()) throws {
        try db.write { database in
            let rows = try Row.fetchAll(database, sql: "SELECT id, summary_json FROM requests WHERE expires_at>? AND status IN ('accepted', 'in_flight')",
                                        arguments: [max(now, Date()).timeIntervalSince1970])
            for row in rows {
                var summary = try JSONDecoder().decode(RequestSummary.self, from: row["summary_json"] as Data)
                summary.status = .interrupted
                summary.delivery = .unknown
                try database.execute(sql: "UPDATE requests SET status=?, summary_json=? WHERE id=?",
                                     arguments: [RequestStatus.interrupted.rawValue, try JSONEncoder().encode(summary), summary.id.uuidString])
            }
        }
    }
    ...
    private func reservedBytes() -> Int64 {
    ...
    private func freeBytes() throws -> Int64 {
        var info = statvfs()
        guard statvfs(path, &info) == 0 else { throw FalconError("storage_unavailable", "Unable to measure free space.", status: 507) }
    ...
    private static func reservationBytes(requestBytes: Int, questionCount: Int) -> Int64 {
        let logical = Int64(FalconLimits.responseBytes) + Int64(requestBytes) * 4 + Int64(questionCount) * 16_384 + 131_072
        return logical * 4
    }
    ...
    private func validate(_ detail: RequestDetail) throws {
        ...
        guard summary.expiresAt == summary.receivedAt.addingTimeInterval(FalconLimits.retention),
              ...
              [summary.timing.bodyReceivedMS, summary.timing.upstreamStartedMS,
               summary.timing.responseReceivedMS, summary.timing.terminalMS,
               summary.timing.deliveryFinishedMS].compactMap({ $0 }).allSatisfy({ $0.isFinite && $0 >= 0 }) else {
            throw FalconError("invalid_record", "Request record exceeds bounds or has invalid timing.")
        }
    }
    ...
    private static func write(_ detail: RequestDetail, database: Database, insert: Bool) throws {
        ...
        if insert {
            try database.execute(sql: "INSERT INTO requests VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                 arguments: [summary.id.uuidString, summary.sourceID.uuidString, summary.profileID.uuidString,
                                             ...
                                             summary.timing.terminalMS, json, detail.receivedRequest,
                                             detail.effectiveRequest, detail.upstreamResponse])
        } else {
            try database.execute(sql: """
                ...
                """, arguments: [summary.status.rawValue, summary.reviewState.rawValue, summary.questionCount,
        ...
        for question in projections {
            try database.execute(sql: "INSERT INTO questions VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                 arguments: [summary.id.uuidString, question.id, question.type, question.fingerprint,
                                             question.result, question.confidence, question.topProbability,
                                             question.margin, question.numericValue])
        }
    }
    ...
    private static func projectQuestions(_ detail: RequestDetail) throws -> [QuestionProjection] {
        guard let data = detail.effectiveRequest,
              let request = try? JSONValue.decode(data),
              let questions = request["questions"]?.objectValue else { return [] }
        ...
        return try questions.map { id, definition in
            ...
            let probabilities = answer?["probabilities"]?.objectValue?.values.compactMap(\.numberValue).sorted(by: >) ??
                                answer?["probabilities"]?.arrayValue?.compactMap(\.numberValue).sorted(by: >) ?? []
            ...
            return QuestionProjection(id: id, type: type,
                                      fingerprint: fingerprint, result: selected,
                                      confidence: answer?["confidence"]?.numberValue,
                                      topProbability: probabilities.first,
                                      margin: probabilities.count > 1 ? probabilities[0] - probabilities[1] : nil,
                                      numericValue: type == "score" ? answer?["score"]?.numberValue :
                                          (type == "noul" ? answer?["noul"]?.numberValue : nil))
        }
    }

    private static func predicate(_ filter: DecisionFilter, before: RequestCursor?, now: Date) throws -> (String, [any DatabaseValueConvertible]) {
        var parts = ["expires_at > ?"]
        var values: [any DatabaseValueConvertible] = [now.timeIntervalSince1970]
        if let since = filter.since { parts.append("received_at >= ?"); values.append(since.timeIntervalSince1970) }
        if let until = filter.until { parts.append("received_at < ?"); values.append(until.timeIntervalSince1970) }
        if !filter.sourceIDs.isEmpty {
            parts.append("source_id IN (\(Array(repeating: "?", count: filter.sourceIDs.count).joined(separator: ",")))")
            values.append(contentsOf: filter.sourceIDs.map(\.uuidString))
        }
        if let status = filter.status { parts.append("status = ?"); values.append(status.rawValue) }
        if let review = filter.reviewState { parts.append("review_state = ?"); values.append(review.rawValue) }
        if let before {
            parts.append("(received_at < ? OR (received_at = ? AND id < ?))")
            values += [before.receivedAt.timeIntervalSince1970, before.receivedAt.timeIntervalSince1970, before.id.uuidString]
        }
        if !filter.search.isEmpty {
            parts.append("(instr(lower(CAST(summary_json AS TEXT)), lower(?)) > 0 OR instr(lower(CAST(received_request AS TEXT)), lower(?)) > 0 OR instr(lower(CAST(effective_request AS TEXT)), lower(?)) > 0 OR instr(lower(CAST(upstream_response AS TEXT)), lower(?)) > 0)")
            values += Array(repeating: filter.search, count: 4)
        }
        var questionParts: [String] = []
        if let fingerprint = filter.questionFingerprint {
            guard fingerprint.count == 64, fingerprint.utf8.allSatisfy({ ($0 >= 48 && $0 <= 57) || ($0 >= 65 && $0 <= 70) || ($0 >= 97 && $0 <= 102) }) else {
                throw FalconError("invalid_filter", "Question fingerprint must be a SHA-256 hex digest.")
            }
            questionParts.append("hex(q.fingerprint)=?")
        ...
        if let band = filter.confidenceBand {
            ...
            questionParts.append("q.type IN ('choice', 'score') AND q.confidence>=? AND q.confidence\(band == 9 ? "<=?" : "<?")")
        ...
        if let band = filter.noulBand {
            ...
            questionParts.append("q.type='noul' AND q.numeric_value>=? AND q.numeric_value\(band == 9 ? "<=?" : "<?")")
            values += [Double(band) / 10, Double(band + 1) / 10]
        }
        if !questionParts.isEmpty {
            parts.append("EXISTS (SELECT 1 FROM questions q WHERE q.request_id=requests.id AND \(questionParts.joined(separator: " AND ")))")
        }
    ...
    private static func profile(_ row: Row) -> UpstreamProfile {
        UpstreamProfile(id: UUID(uuidString: row["id"] as String)!, name: row["name"],
                        baseURL: row["base_url"], defaultModel: row["default_model"],
                        revision: row["revision"], credentialID: row["credential_id"], enabled: row["enabled"])
    }

    private static func source(_ row: Row) -> AgentSource {
        AgentSource(id: UUID(uuidString: row["id"] as String)!, name: row["name"],
    ...
    private static func key(_ row: Row) -> SourceKey {
        ...
        return SourceKey(id: UUID(uuidString: row["id"] as String)!, sourceID: UUID(uuidString: row["source_id"] as String)!,
                         digest: row["digest"], suffix: row["suffix"],
