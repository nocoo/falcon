...
品牌区高 72 pt，名称 18 pt；44 pt 原图仅在布局上收紧右侧透明区域到 32 pt，保留实际鸟头。版本与名称基线对齐。Settings 归入 Manage 导航，底部只保留运行状态卡。

成功和失败使用更饱和的绿、红 token；失败行、详情徽章、错误信息和时间线统一用红色，警告保留琥珀色。运行指示灯使用独立亮绿 token，以 1.8 秒单程缓动轻微呼吸并带光晕；只在服务运行、界面活跃且未开启减少动态效果时动画。减少透明度或增强对比度时关闭光晕；状态始终同时显示文字，不依赖颜色传达。
