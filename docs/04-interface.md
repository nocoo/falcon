...
请求接收落盘后立即进入列表，无需等待 Jev 完成。数据库提交驱动状态原位更新；新记录不抢选中、草稿或滚动锚点，显示 `12 new decisions` 入口可回到最新记录。阅读旧记录和窗口失焦时仍接收更新；Pause service 才停止新推理。超过一页的突发请求保留分页游标，继续加载时不跳过中间记录。
